...
window = sg.Window('Youtube Downloader by Nadet', start_layout)
# video_link = "https://www.youtube.com/watch?v=PtWmQiqwixY"
...
while True:
    ...
    if event == "Submit":
        ...
        # video info
        window = sg.Window('Youtube Downloader by Nadet',
                           layout, finalize=True)
        window['-TITLE-'].update(video_object.title)
        window['-LENGTH-'].update(
            f'{round(video_object.length / 60, 2)} minutes')
